...
class _DummySession:
    ...
    ) -> None:  # noqa: D401 - signature mirrors aiohttp
        self.args = args
        self.kwargs = kwargs
        self.closed = False
    ...
    ) -> None:  # noqa: D401 - standard context manager
        self.closed = True


@pytest.mark.asyncio
async def test_collect_indicator_snapshots_filters_failures(
    ...
    monkeypatch.setattr(
        ai_service_module.Config, "API_BASE_URL", "http://example.com", raising=False
    )
    ...
    async def fake_fetch(
        self: AIService, session: Any, url: str, params: dict[str, str]
    ) -> dict[str, Any]:
...
@pytest.mark.asyncio
async def test_looks_like_forex_pair_and_resolution(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = AIService()
