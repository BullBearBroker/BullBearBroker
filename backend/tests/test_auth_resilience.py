...
# QA: marcamos este módulo como rate_limit por probar escenarios de login intensivos
...
@pytest.mark.asyncio
async def test_login_with_invalid_credentials_remains_unauthorized(
    client: AsyncClient,
) -> None:
...
async def test_login_third_invalid_triggers_backoff(client: AsyncClient) -> None:
    ...
    for attempt in range(2):
        ...
        ), f"Attempt {attempt + 1} should not be rate limited"
...
@pytest.mark.asyncio
async def test_refresh_with_corrupted_token_returns_unauthorized(
    client: AsyncClient,
) -> None:
    response = await client.post(
        "/api/auth/refresh",
...
@pytest.mark.asyncio
async def test_revoke_all_refresh_tokens_invalidates_existing_ones(
    client: AsyncClient,
) -> None:
...
async def test_login_returns_service_unavailable_on_dependency_failure(
    ...
    assert any(
        ...
        and evt.get("dependency") == "database"
        for evt in events
    )
