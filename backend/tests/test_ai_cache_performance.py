...
# QA: marcamos este módulo slow por depender de contadores globales de métricas
...
def test_adaptive_timeout_logging(monkeypatch, caplog):
    ...
    try:
        ...
        assert ai_adaptive_timeouts_total._value.get() == pytest.approx(baseline + 1)
        assert any(
            "adaptive_timeout_triggered" in record.message for record in caplog.records
        )
