...
from backend.services import (
    ai_service as ai_service_module,
    mistral_service as mistral_module,
)
...
async def test_ai_provider_metrics_and_logs(monkeypatch, caplog):
    ...
    monkeypatch.setattr(mistral_module.aiohttp, "ClientSession", FakeClientSession)
    monkeypatch.setattr(ai_service_module.aiohttp, "ClientSession", FakeClientSession)
    ...
    assert any(
        entry.get("provider") == "HuggingFace" and entry.get("status") == "ok"
        for entry in provider_calls
    )
    assert any(
