...
@pytest.mark.anyio
async def test_resolve_price_ignores_invalid_provider_payloads(
    service: PortfolioService, monkeypatch: pytest.MonkeyPatch
) -> None:
...
@pytest.mark.anyio
async def test_resolve_price_returns_none_when_all_sources_fail(
    service: PortfolioService, monkeypatch: pytest.MonkeyPatch
) -> None:
