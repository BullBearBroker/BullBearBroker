...
class DummySession:
    ...
    ) -> None:  # noqa: D401 - simple context manager
        return None
...
@pytest.mark.anyio
async def test_chat_completion_fallbacks_to_small_on_429(
    service: MistralService, monkeypatch: pytest.MonkeyPatch
) -> None:
...
@pytest.mark.anyio
async def test_chat_completion_uses_large_after_server_errors(
    service: MistralService, monkeypatch: pytest.MonkeyPatch
) -> None:
    async def fake_request(session, headers, payload):  # noqa: ANN001
        model = payload["model"]
...
@pytest.mark.anyio
async def test_analyze_market_sentiment_handles_corrupt_payload(
    service: MistralService, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        service, "chat_completion", AsyncMock(return_value="{not json}")
    )
...
@pytest.mark.anyio
async def test_chat_completion_raises_clear_error_when_all_models_fail(
    service: MistralService, monkeypatch: pytest.MonkeyPatch
) -> None:
    async def fake_request(session, headers, payload):  # noqa: ANN001
