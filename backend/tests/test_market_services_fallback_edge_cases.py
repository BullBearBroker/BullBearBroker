...
@pytest.mark.asyncio
async def test_crypto_service_falls_back_when_primary_returns_none(
...
@pytest.mark.asyncio
async def test_forex_service_skips_providers_without_keys(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    service = ForexService(
        cache_client=DummyCache(), session_factory=lambda timeout=None: AsyncMock()
    )
...
@pytest.mark.asyncio
async def test_stock_service_handles_corrupt_provider_payload(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    service = StockService(
        cache_client=cache, session_factory=lambda timeout=None: AsyncMock()
    )
