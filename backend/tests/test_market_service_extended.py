...
def market_service(monkeypatch: pytest.MonkeyPatch) -> MarketService:
    monkeypatch.setattr(
        market_service_module, "CacheClient", lambda *args, **kwargs: _StubCache({})
    )
    return MarketService()


@pytest.mark.asyncio
async def test_fetch_binance_history_normalizes_payload(
...
@pytest.mark.asyncio
async def test_get_historical_ohlc_falls_back_to_yahoo(
    monkeypatch: pytest.MonkeyPatch, market_service: MarketService
) -> None:
...
@pytest.mark.asyncio
async def test_get_historical_ohlc_raises_when_yahoo_returns_error(
    monkeypatch: pytest.MonkeyPatch, market_service: MarketService
) -> None:
    async def failing_binance(*_args, **_kwargs):
        raise ClientError("binance down")
...
@pytest.mark.asyncio
async def test_fetch_binance_history_raises_for_corrupted_payload(
    monkeypatch: pytest.MonkeyPatch, market_service: MarketService
) -> None:
    payload = [["bad-entry"]]
    dummy_response = _DummyResponse(status=200, payload=payload)
...
@pytest.mark.asyncio
async def test_history_cache_hit_skips_fetch(
    monkeypatch: pytest.MonkeyPatch, market_service: MarketService
) -> None:
    calls: list[str] = []
...
@pytest.mark.asyncio
async def test_fetch_binance_history_rejects_invalid_interval(
...
async def test_get_historical_ohlc_raises_for_unsupported_interval(
    ...
    with pytest.raises(ValueError) as exc:
        await market_service.get_historical_ohlc(
            "btcusdt", interval="7m", market="crypto"
        )
...
async def test_get_historical_ohlc_falls_back_when_binance_fails_with_asyncmock(
    ...
    monkeypatch.setattr(
        MarketService, "_looks_like_crypto", lambda *_args, **_kwargs: True
    )

    result = await market_service.get_historical_ohlc(
        "aapl", interval="1h", market="auto"
    )
...
async def test_get_historical_ohlc_uses_cached_value_when_available(
    ...
    result = await market_service.get_historical_ohlc(
        "ETHUSDT", interval="1h", market="auto"
    )
...
async def test_get_historical_ohlc_returns_empty_dataset_when_providers_fail(
    ...
    monkeypatch.setattr(MarketService, "_fetch_yahoo_history", yahoo_mock)
    monkeypatch.setattr(
        MarketService, "_looks_like_crypto", lambda *_args, **_kwargs: False
    )

    result = await market_service.get_historical_ohlc(
        "AAPL", interval="1h", market="stock"
    )
...
@pytest.mark.asyncio
async def test_get_price_history_fetches_and_caches(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "chart": {
    ...
    history_cached = await market_service.get_price_history(
        "AAPL", interval="1h", range_="1mo"
    )
...
@pytest.mark.asyncio
async def test_get_price_history_raises_on_error_status(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_get_price_history_raises_when_payload_incomplete(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {"chart": {"result": [{}]}}
    dummy_response = _DummyResponse(status=200, payload=payload)
...
@pytest.mark.asyncio
async def test_fetch_binance_history_raises_on_http_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_fetch_yahoo_history_parses_payload(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "chart": {
...
@pytest.mark.asyncio
async def test_fetch_yahoo_history_skips_none_entries(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "chart": {
...
@pytest.mark.asyncio
async def test_get_historical_ohlc_returns_binance_data(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    payload = {
        "symbol": "BTCUSDT",
    ...
    result = await market_service.get_historical_ohlc(
        "btcusdt", interval="1h", market="crypto"
    )
...
@pytest.mark.asyncio
async def test_get_historical_ohlc_raises_when_no_providers_available(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    market_service = MarketService()
    market_service.history_cache = _StubCache({})  # type: ignore[assignment]
    ...
    with pytest.raises(ValueError) as exc:
        await market_service.get_historical_ohlc(
            "btcusdt", interval="1h", market="auto"
        )
...
@pytest.mark.asyncio
async def test_get_price_formats_crypto_payload(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
    monkeypatch.setattr(
        MarketService, "detect_asset_type", AsyncMock(return_value="crypto")
    )
    crypto_payload = {
        "price": 100.0,
        "raw_change": 5.0,
    ...
    monkeypatch.setattr(
        MarketService, "get_crypto_price", AsyncMock(return_value=crypto_payload)
    )
...
async def test_get_price_formats_stock_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    service = MarketService()
    monkeypatch.setattr(
        MarketService, "detect_asset_type", AsyncMock(return_value="stock")
    )
    stock_payload = {
        "price": 50.0,
        "raw_change": -1.25,
    ...
    monkeypatch.setattr(
        MarketService, "get_stock_price", AsyncMock(return_value=stock_payload)
    )
...
@pytest.mark.asyncio
async def test_get_crypto_price_merges_binance_data(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
    ...
    monkeypatch.setattr(
        MarketService, "get_binance_price", AsyncMock(return_value=binance_payload)
    )
...
@pytest.mark.asyncio
async def test_get_crypto_price_returns_none_when_no_data(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
    service.crypto_service.get_price = AsyncMock(return_value=None)  # type: ignore[assignment]
    monkeypatch.setattr(
        MarketService, "get_binance_price", AsyncMock(return_value=None)
    )
...
@pytest.mark.asyncio
async def test_get_stock_price_handles_non_numeric_change(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
...
@pytest.mark.asyncio
async def test_get_stock_market_data_filters_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
...
@pytest.mark.asyncio
async def test_get_crypto_market_data_filters_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
...
@pytest.mark.asyncio
async def test_process_market_data_combines_lists(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
    stock_data = [
    ...
    crypto_data = {
        "top_gainers": [
            {"symbol": "BTC", "price": 200.0, "raw_change": 5.0, "type": "crypto"}
        ],
        "top_losers": [
            {"symbol": "ETH", "price": 150.0, "raw_change": -4.0, "type": "crypto"}
        ],
    }
...
@pytest.mark.asyncio
async def test_get_top_performers_uses_real_data(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
    ...
    monkeypatch.setattr(
        MarketService,
        "get_binance_top_performers",
        AsyncMock(return_value=binance_payload),
    )
    monkeypatch.setattr(
        MarketService, "get_stock_market_data", AsyncMock(return_value=stock_payload)
    )
...
@pytest.mark.asyncio
async def test_get_top_performers_falls_back_to_simulated(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = MarketService()
    ...
    monkeypatch.setattr(
        MarketService, "get_stock_market_data", AsyncMock(return_value=[])
    )
    monkeypatch.setattr(
        MarketService,
        "process_market_data",
        AsyncMock(side_effect=ValueError("no data")),
    )
    monkeypatch.setattr(
        MarketService, "get_simulated_data", AsyncMock(return_value=simulated)
    )
...
async def test_get_news_uses_cache_and_rss(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        market_service_module.Config, "MEDIASTACK_API_KEY", "", raising=False
    )
    ...
    monkeypatch.setattr(
        MarketService, "_fetch_rss", AsyncMock(return_value=rss_payload)
    )
