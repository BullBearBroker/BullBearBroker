...
# QA: marcamos este módulo slow por procesar datasets grandes en xdist
...
def test_resample_series_rejects_invalid_interval() -> None:
    with pytest.raises(ValueError):
        resample_series([{"timestamp": datetime.now(UTC), "close": 1.0}], "15m")


def test_resample_series_collapses_duplicate_buckets() -> None:
    base = datetime(2024, 1, 1, 12, 15, tzinfo=UTC)
    series = [
