...
def _encode_token(payload: dict[str, object]) -> str:
    return jwt.encode(
        payload, security_module.ACCESS_SECRET, algorithm=security_module.JWT_ALG
    )
...
def test_extract_identity_validates_uuid(service: UserService) -> None:
    ...
    sample_id = uuid4()
    user_id, email = UserService._extract_identity(
        {"user_id": str(sample_id), "email": "user@example.com"}
    )
...
def test_create_session_rejects_token_with_mismatched_email(
    service: UserService,
) -> None:
...
def test_store_refresh_token_accepts_invalid_payload(
    service: UserService, session_factory
) -> None:
