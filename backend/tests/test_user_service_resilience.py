...
def test_create_user_with_invalid_risk_profile_does_not_touch_db(
...
def test_rotate_refresh_token_rejects_expired_entry(
    service: UserService, session_factory
) -> None:
    ...
    with session_factory() as session:
        ...
        db_token.expires_at = datetime.now(UTC) - timedelta(minutes=5)
        session.commit()
...
def test_revoke_all_refresh_tokens_clears_memory_even_when_empty(
    service: UserService, session_factory
) -> None:
