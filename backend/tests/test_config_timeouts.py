...
def test_database_engine_respects_pool_configuration(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_timeseries_http_timeout_respects_configuration(
    ...
    monkeypatch.setattr(timeseries_service, "_HTTP_CONNECT_TIMEOUT_SECONDS", 2.0, raising=False)
    ...
    monkeypatch.setattr(timeseries_service.httpx, "AsyncClient", lambda **kwargs: DummyClient(kwargs.get("timeout")))
