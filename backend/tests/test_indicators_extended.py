...
def test_indicators_handle_extreme_values() -> None:
    ...
    vwap_value = indicators.volume_weighted_average_price(
        highs, lows, closes, [100] * len(values)
    )
...
def test_vwap_returns_none_with_zero_volume() -> None:
    ...
    volumes = [0, 0]
    assert (
        indicators.volume_weighted_average_price(highs, lows, closes, volumes) is None
    )
...
def test_indicators_handle_extreme_large_values_consistently() -> None:
    ...
    vwap_value = indicators.volume_weighted_average_price(
        highs, lows, closes, [1] * len(values)
    )
