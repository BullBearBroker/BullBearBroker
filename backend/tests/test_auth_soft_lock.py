...
# QA: marcamos este módulo como rate_limit por sensibilidad a ventanas temporales
...
async def test_soft_lock_triggers_and_resets(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "backend.routers.auth._LOGIN_BACKOFF_START_AFTER", 10, raising=False
    )
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        soft_lock_events = [
            evt for evt in events if evt.get("event") == "account_soft_lock"
        ]
