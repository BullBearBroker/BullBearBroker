...
async def test_process_message_empty_prompt_triggers_local_fallback(
    ...
    async def mistral_failure(
        message: str, context: dict[str, Any]
    ) -> str:  # noqa: ARG001
        raise RuntimeError("provider down")

    async def huggingface_failure(
    ...
    monkeypatch.setattr(
        ai_service_module.asyncio, "sleep", AsyncMock(return_value=None)
    )
...
@pytest.mark.anyio
async def test_call_huggingface_raises_for_unexpected_payload(
    ...
    monkeypatch.setattr(
        ai_service_module.aiohttp, "ClientSession", lambda: DummySession()
    )
...
@pytest.mark.anyio
async def test_call_with_backoff_tracks_invalid_provider(
    service: AIService, monkeypatch: pytest.MonkeyPatch
) -> None:
