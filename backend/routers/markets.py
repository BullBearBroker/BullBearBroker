...
DEFAULT_FOREX_ORDER = ["Twelve Data", "Yahoo Finance"]
ORDER = getattr(Config, "FOREX_FALLBACK_ORDER", DEFAULT_FOREX_ORDER)


def _apply_forex_order_from_config() -> None:
    available = list(getattr(forex_service, "apis", ()))
    if not available:
        return

    configured = [
        name for name in ORDER if any(api.get("name") == name for api in available)
    ]
    if not configured:
        configured = DEFAULT_FOREX_ORDER

    ordered = [
        api for name in configured for api in available if api.get("name") == name
    ]
    remaining_names = {api.get("name") for api in ordered}
    remaining = [api for api in available if api.get("name") not in remaining_names]
    forex_service.apis = tuple(ordered + remaining)
...
async def get_forex(pair: str) -> dict[str, Any]:
    ...
    try:
        _apply_forex_order_from_config()
