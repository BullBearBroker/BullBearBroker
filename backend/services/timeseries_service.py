...
def _normalize_point(
    ...
    if isinstance(point, Mapping):
        timestamp_raw = point.get("timestamp") or point.get("time") or point.get("date")
        if timestamp_raw is None:
            raise ValueError("Cada dato debe incluir 'timestamp'")
        price_raw = (
            point.get("close") if point.get("close") is not None else point.get("price")
        )
...
async def get_crypto_closes_binance(
    ...
    ]  # [Codex] nuevo - capturamos precios de apertura
...
async def get_stock_closes(
    ...
    if TWELVEDATA_API_KEY:
        ...
        try:
            ...
            if "values" in data:
                ...
                highs = [
                    float(v.get("high", v["close"])) for v in data["values"]
                ]  # [Codex] nuevo
                lows = [
                    float(v.get("low", v["close"])) for v in data["values"]
                ]  # [Codex] nuevo
                opens = [
                    float(v.get("open", v["close"])) for v in data["values"]
                ]  # [Codex] nuevo
                volumes = [
    ...
    if interval == "1h":
        ...
        if key not in data:
            error_detail = (
                data.get("Note") or data.get("Error Message") or "respuesta inválida"
            )
        ...
        highs = [float(v.get("2. high", v["4. close"])) for _, v in items][
            -limit:
        ]  # [Codex] nuevo
        lows = [float(v.get("3. low", v["4. close"])) for _, v in items][
            -limit:
        ]  # [Codex] nuevo
        opens = [float(v.get("1. open", v["4. close"])) for _, v in items][
            -limit:
        ]  # [Codex] nuevo
        volumes = [float(v.get("5. volume", 0.0) or 0.0) for _, v in items][
            -limit:
        ]  # [Codex] nuevo
        return closes, {
