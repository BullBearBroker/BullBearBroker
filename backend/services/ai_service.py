...
def _should_decorate_market() -> bool:
    env_flag = os.getenv("BULLBEAR_AI_DECORATE")
    if env_flag is not None and not os.getenv("PYTEST_CURRENT_TEST"):
        try:
            return bool(int(env_flag))
        except ValueError:
            return bool(env_flag)
    return bool(getattr(Config, "AI_DECORATE_MARKET", True))
...
class AIService:
    ...
    async def process_message(
        ...
        async def _execute_prompt() -> AIResponsePayload:
            ...
            elif not text and market_summary:
                text = market_summary
    ...
    async def _call_huggingface(self, message: str, context: dict[str, Any]) -> str:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if status_code != 200:
                        ...
                        error.ai_reason = reason
                        error.ai_status_code = status_code
                        raise error
                    ...
                    try:
                        ...
                        value_error.ai_reason = reason
                        value_error.ai_status_code = status_code
            ...
            timeout_error.ai_reason = reason
            timeout_error.ai_status_code = 408
            ...
            runtime_error.ai_reason = reason
            runtime_error.ai_status_code = None
...
¿Hay algo más en lo que pueda ayudarte?"""
    ...
    async def stream_generate(self, prompt: str):
        ...
        try:
            ...
            if not used_cache and prompt_key and chunks_collected:
