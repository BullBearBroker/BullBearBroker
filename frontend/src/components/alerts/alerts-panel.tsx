...
import { useAlertsWebSocket, type AlertWebSocketEvent } from "@/hooks/useAlertsWebSocket";
...
export function AlertsPanel({ token }: AlertsPanelProps) {
  const { data, error, mutate, isLoading } = useSWR<Alert[]>(token ? ["alerts", token] : null, () =>
    listAlerts(token!),
  );
  ...
  const handleLiveAlert = useCallback((event: AlertWebSocketEvent) => {
    ...
    setLiveAlerts((prev) => {
      const entry: LiveAlertEntry = {
        ...
        message:
          typeof event.message === "string" ? event.message : "Se recibió una alerta en vivo",
  ...
  const handleCreate = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        console.error(err);
        setFormError(
          err instanceof Error ? err.message : "No se pudo crear la alerta. Inténtalo de nuevo.",
        );
      } finally {
        setSubmitting(false);
      }
  ...
  const toggleAlert = useCallback(
    async (alert: Alert) => {
      ...
      try {
        ...
        console.error(err);
        setFormError(err instanceof Error ? err.message : "No se pudo actualizar la alerta.");
      }
    },
    [mutate, token],
  );
  ...
  const handleSuggest = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setSuggestError(
        err instanceof Error ? err.message : "No se pudo obtener la sugerencia de la IA.",
      );
    } finally {
      setSuggesting(false);
    }
  ...
  const removeAlert = useCallback(
    async (alert: Alert) => {
      ...
      try {
        ...
        console.error(err);
        setFormError(err instanceof Error ? err.message : "No se pudo eliminar la alerta.");
      }
    },
    [mutate, token],
  );
  ...
  const handleSendNotification = useCallback(
    async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        console.error(err);
        setDispatchError(err instanceof Error ? err.message : "No se pudo enviar la notificación.");
      } finally {
    ...
    <Card className="flex flex-col">
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2 rounded-lg border border-dashed p-3">
          ...
          {wsSystemMessage && <p className="text-xs text-muted-foreground">{wsSystemMessage}</p>}
          {liveAlerts.length > 0 ? (
              ...
              {liveAlerts.map((item) => (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {typeof item.price === "number" ? ` · Precio ${item.price.toFixed(2)}` : ""}
                    {typeof item.target === "number" ? ` · Objetivo ${item.target.toFixed(2)}` : ""}
                  </p>
                </li>
              ))}
            </ul>
        ...
        <form onSubmit={handleCreate} className="space-y-3">
          ...
          {suggestNote && <p className="text-xs text-muted-foreground">{suggestNote}</p>}
        ...
        <div className="space-y-3">
          ...
          {error && (
            <p className="text-sm text-destructive">
              Error al cargar alertas: {error instanceof Error ? error.message : "Desconocido"}
            </p>
          )}
